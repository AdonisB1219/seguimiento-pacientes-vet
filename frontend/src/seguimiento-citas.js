...
export class SeguimientoCitas extends LitElement{
    ...
    async handleDatosActualizados(event){
        if (this.banderaEditar){
            ...
            await fetch(`http://localhost:8081/api-citas/${datos.id}`, requestOptions)
            ...
            await fetch("http://localhost:8081/api-citas", requestOptions)
                .then(response => response.json())
                .catch(error => console.log('error', error));
            this.getData();
        }
    ...
    async getData(){
        ...
        const result = await fetch("http://localhost:8081/api-citas", {method: 'GET'})
        .then(response => response.json())
        .catch(error => console.log('error', error));
